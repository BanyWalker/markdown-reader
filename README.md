...
当前最新版（`0.1.7`）：[下载 Windows x64 安装包](./release/MD-Reader-latest-x64-setup.exe)。每次成功构建都会更新该固定文件名，同时在 `release/` 中新增不可覆盖的版本归档 `MD-Reader-<版本号>-x64-setup.exe`；之前的版本归档会保留。正式发布后也会同步到 GitHub Releases（或项目配置的发布渠道）。
