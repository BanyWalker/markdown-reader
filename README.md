...
- **本地优先**：通过系统文件选择器逐个打开文件，并以标签页在同一窗口中切换；目录侧栏会保留不同目录的项目，递归加载文档目录，并在目录内容变化后自动刷新。
...
- **阅读状态**：按文件记忆滚动位置和最近打开记录，重启应用后仍可恢复；窗口位置、尺寸和最大化/全屏状态也会记忆。
...
切换文件、文件夹或关闭窗口时，如果当前文档有未保存修改，应用会提供“保存 / 不保存 / 取消”选项。打开的文件被其他程序修改后会自动检查；编辑状态下可选择继续编辑、丢弃当前修改或覆盖当前文档，避免静默覆盖外部更新。

## 下载与安装

当前最新版（`0.1.10`）：[下载 Windows x64 安装包](./release/MD-Reader-latest-x64-setup.exe)。每次成功构建都会更新该固定文件名，同时在 `release/` 中新增不可覆盖的版本归档 `MD-Reader-<版本号>-x64-setup.exe`；之前的版本归档会保留。正式发布后也会同步到 GitHub Releases（或项目配置的发布渠道）。
